-- Alpha Hub Calendar backend stabilization, safe paste version.
-- This version avoids comma-chained ALTER TABLE statements.
...
declare
  ...
  body text;
begin
  prefix := case when lower(coalesce(chain_name, '')) like '%base%' then 'BASE' else 'AH' end;
...
with duplicate_codes as (
  select id, row_number() over (partition by share_code order by created_at, id) as rn
  from public.calendar_projects
  where share_code is not null
),
duplicate_slugs as (
  select id, row_number() over (partition by share_slug order by created_at, id) as rn
  from public.calendar_projects
...
set
  ...
  share_slug = case when coalesce(ds.rn, 1) > 1 then cp.share_slug || '-' || lower(substr(replace(cp.id::text, '-', ''), 1, 4)) else cp.share_slug end
...
do $$
begin
  alter table public.wl_projects add column if not exists calendar_project_id uuid references public.calendar_projects(id) on delete set null;
  alter table public.wl_projects add column if not exists share_code text;
  alter table public.wl_projects add column if not exists image_url text;
  alter table public.wl_projects add column if not exists mint_time_source text;
  alter table public.wl_projects add column if not exists mint_time_confidence text;
  alter table public.wl_projects add column if not exists mint_time_confirmed boolean default false;
  ...
  alter table public.wl_projects add column if not exists max_total_spend numeric;
...
do $$
begin
  alter table public.wl_projects
